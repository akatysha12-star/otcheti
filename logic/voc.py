"""VOC: вся логика расчётов без Streamlit.

Один файл = одна задача: парсинг, свод, Excel.
Стиль страниц и бренд живут в config/, styles.py, pages/.
"""
...
# Стандартный список номеров ресторанов
...
def canonical_block_key(value: str) -> str:
    """Ключ блока для сопоставления: только буквы и цифры."""
    value = str(value or "")
...
def beautify_item(s: str) -> str:
    """Лёгкое приведение текста к читаемому виду. Не меняет смысл пункта."""
    s = re.sub(r"(?<=[^\s])\(", " (", s)
    s = re.sub(r"\(\s+", "(", s)
    s = re.sub(r"\s+\)", ")", s)
    s = re.sub(r",(?=\S)", ", ", s)
...
def clean_item(s: str) -> str:
    """Очищает текст пункта чек-листа, сохраняя смысл максимально полностью."""
    s = str(s or "")
    s = s.replace("\\*", "*")
    s = re.sub(r"\s+", " ", s).strip()
...
def parse_voc_text(text: str, filename: str):
    ...
    Возвращает: restaurant_num, restaurant_name, results, где
    results: {(block, normalized_item_key): {"value": 0/1, "display": str}}
    """
    ...
    def add_result(block_name: str, item_text: str, result_word: str):
        ...
        if key not in results:
            results[key] = {"value": value, "display": item_clean}
        else:
            if value == 1:
                results[key]["value"] = 1
...
def parse_files(files):
    ...
    for f in files:
        try:
            ...
            parsed.append({"num": num, "name": name, "results": results, "file": f.name})
...
def build_summary(parsed, include_default=False, only_uploaded=True):
    """Собирает свод: блоки, рестораны, матрицу нарушений.

    В свод попадают только пункты, где есть хотя бы один НЕТ.
    """
    blocks = OrderedDict()
    ...
    def ensure_restaurant(num, name):
        ...
        if label not in rest_info:
            rest_info[label] = {"num": num, "name": name}
        else:
...
def generate_excel(blocks, rest_labels, rest_info, matrix):
    """Создает Excel-файл через openpyxl. Возвращает байты."""
    wb = Workbook()
    ws = wb.active
    ws.title = "VOC свод"
...
def excel_file_name() -> str:
    """Имя файла свода с датой по Москве."""
    now_msk = datetime.now(MSK)
    return f"VOC_Summary_Report_{now_msk.strftime('%Y-%m-%d')}.xlsx"
