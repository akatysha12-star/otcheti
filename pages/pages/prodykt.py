...
def create_combo_sheet(wb, all_data):
    ws = wb.create_sheet("Комбо")
    thin_border = Border(left=Side(style='thin'), right=Side(style='thin'), top=Side(style='thin'), bottom=Side(style='thin'))
    combo_data = all_data[all_data['Категория_отчёт'] == 'комбо и радости'].copy()
    combo_data['Город'] = combo_data['Юридическое лицо'].apply(map_city)
    combo_data = combo_data[combo_data['Город'].notna()]
    combo_data = combo_data[~combo_data['Блюдо_очищенное'].str.lower().str.contains('мастер', na=False)]
    ...
    city_data_1 = {}
    for city in ['СПБ', 'Тюмень']:
        city_df = combo_data[combo_data['Город'] == city].copy()
        total_qty = city_df['Количество блюд'].sum()
        grouped = city_df.groupby('Блюдо_очищенное').agg({'Количество блюд': 'sum', 'Сумма со скидкой, р.': 'sum'}).reset_index()
        grouped['%_qty'] = grouped['Количество блюд'] / total_qty if total_qty > 0 else 0
        grouped = grouped.sort_values('%_qty', ascending=False).reset_index(drop=True)
        city_data_1[city] = grouped
    ...
    for city in ['СПБ', 'Тюмень']:
        ...
        grouped = city_df.groupby('Блюдо_очищенное').agg({'Количество блюд': 'sum', 'Сумма со скидкой, р.': 'sum'}).reset_index()
        grouped['%_sum'] = grouped['Сумма со скидкой, р.'] / total_sum if total_sum > 0 else 0
        grouped = grouped.sort_values('%_sum', ascending=False).reset_index(drop=True)
        city_data_2[city] = grouped
    ...
    if st.button("🚀 Сгенерировать отчет", type="primary"):
        with st.spinner("⏳ Формирую отчет..."):
            try:
                ...
                create_combo_sheet(wb, all_data)
